...
如果IP地址是动态的，可以尝试关机-启动虚拟机
主页-》虚拟机-》勾选自己的虚拟机-》选择停止-》选择启动

去Azure云重新创建一个IP地址
1. 在Azure 所有资源中找到自己的网络接口， 在设置里面选择IP配置，右侧会列出当前生效的IP配置，点击该IP，弹出配置窗口，点击IP地址，选择新建一个ip，之后保存配置  
2. 登录VPN Server，https://你的VPN-server-ip:943/Admin， 查看是否连接正常
3. download最新的client profile， 重新配置client端登录