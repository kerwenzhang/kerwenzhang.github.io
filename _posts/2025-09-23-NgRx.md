...
app/store/reducers/counter.reducer.ts, 初始状态：    
...
app/store/index.ts中已经自动注册了State和reducer  
...
修改counter.reducer.ts，定义State接口里必须有一个`number`类型的count，初始状态下count为0。reducer中除了initialState外，增加两个on方法接收action返回一个全新的State：    
...
on函数就收两个参数，一个是action的名字，第二个参数是回调函数，用来处理state，拿到原先的state，然后state.count加1  

修改app.component.ts分发消息    
...
在counter.reducer.ts中，将action作为第二个参数传入，我们就可以使用action中的count  
...
在store\index.ts中，我们定义一个MetaReducer, 它接收一个Reducer, 并返回一个Reducer，在返回的过程中我们可以加一些处理，比如把前后的State都打印出来：    
...
# Selectors  
选择器（Selectors）是用于从 Store 状态中获取数据， selector是为了把获取数据的复杂过程从component代码中分离出来，简化数据获取的流程，component只需要访问selector就能获取数据。@ngrx/store 提供了一些辅助函数来优化这种选择过程。选择器在选择状态片段时具备以下优势：
+ 可移植性（Portability）  
+ 记忆化（Memoization）
+ 组合性（Composition）
+ 可测试性（Testability）
...
store/selectors/counter.selector.ts默认引入了    

    import { createFeatureSelector, createSelector } from '@ngrx/store';

createFeatureSelector是用来获取最外层的State， createSelector是用来获取里层的State  
...
使用以下命令行创建effect
    ...
    ng g effect store/effects/counter --root -m ../../app.module.ts
