...
# Anomaly CVBBATCH-2335 Missing SafeSEH
There is a DFS issue found CVBBATCH-2335 FactoryTalk Batch Enterprise Integration Server - Missing SafeSEH.   
This issue only occurs in x86 exe, 64 bit exe does not have this flag.  
Batch EI Server and Batch View Server use pkg to package js files into exe. When do packaging, PKG appends our source files at the end of node.exe, then unzip it when execute. So what we need to do is to add SafeSEH flag for node.exe.    
