...
6. 通过使用 以下选项将blob注入到hello.exe中：
    ...
    + NODE_SEA_BLOB- 二进制文件中资源/注释/部分的名称 Blob 的内容将存储在其中。
    + sea-prep.blob- 在步骤 1 中创建的 blob 的名称。
    + --sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2- Node.js 项目用于检测文件是否已注入的fuse。
...
## 生成blob
blob 可以 使用 Node.js 二进制文件的标志生成？。它采用JSON 格式的配置文件。如果传递给它的路径不是绝对路径，则 Node.js将使用相对路径
...
## V8 代码缓存支持
当设置此选项时，在生成期blob期间，Node.js将编译脚本以生成 V8 代码缓存。生成的代码缓存将是blob的一部分，并最终注入到可执行文件exe中。当应用程序启动时，Node.js不会从头开始编译script，而是使用代码缓存来加快编译速度，这将提高启动性能。

注意：当 import()useCodeCache为true时不起作用。？
...
require()在注入的主脚本中，它与未注入的模块可用的 require（） 不同。它也没有任何 非注入的 require（） 具有的属性，除了 require.main。它 只能用于加载内置模块。尝试加载一个 only be found in file system 的模块，将引发错误。

用户无需依赖基于文件，而是可以将他们的 application 导入到独立的 JavaScript 文件中，以注入到可执行文件中。 这还可以确保更确定的依赖关系图。
