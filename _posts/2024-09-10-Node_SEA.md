...
Server端  
1. 创建一个Server文件夹，搭一个express。在所有路由之前，添加static
...
2. 运行命令行`node app.js`， 确保自己能正确访问到Angular前端  
...
4. 后续操作和之前demo类似，先用esbuild将所有server文件bundle到一起。运行命令行 `npm run build`，将所有的文件bundle到out.js
5. 生成blob   
