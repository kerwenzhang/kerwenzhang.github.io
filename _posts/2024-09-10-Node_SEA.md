...
Server端  
1. 创建一个Server文件夹，搭一个express。在所有路由之前，添加静态资源

        app.use('/', express.static(path.join(__dirname, './../client/dist/client/browser')));

2. 运行命令行`node app.js`， 确保自己能正确访问Angular前端页面  
3. 修改SEA配置文件，添加assets
...
4. 后续操作和之前demo类似，先用`esbuild`将所有server文件bundle到一起。运行命令行 `npm run build`，生成out.js
5. 生成blob   
...
9. 双击运行express.exe，打开浏览器输入http://localhost:8888  
![img](https://github.com/kerwenzhang/kerwenzhang.github.io/blob/master/_posts/image/sea3.png?raw=true)，可以看到toggle和basic button能正确显示  
