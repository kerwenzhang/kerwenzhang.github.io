...
# 要求
1 支持加载 Node.js 本机附加组件
2 将涉及 I/O 的 Node.js API 函数调用代理回 VFS
3 支持从 VFS 内部运行可执行程序
4 将命令行参数代理到嵌入在 VFS 中的程序
5 将环境变量代理到嵌入在 VFS 中的程序
在 Node.js 的上下文中，社区历来被迫对 fs 等模块进行 monkey-patch，以用于高级 I/O 用例。例如，Electron 对几个 Node.js 模块进行 monkey-patch，以支持其 ASAR 集成。Vercel 的 PKG 以类似的方式对各种功能进行 monkey-patch。这种方法不仅容易出错，而且如果 Node.js 阻止对其内部模块进行运行时修改（即出于安全原因），那么将来可能就无法实现。
