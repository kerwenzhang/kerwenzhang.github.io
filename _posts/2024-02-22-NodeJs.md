...
4. 在 src 目录下创建一个新文件，例如 addon.cc    
5. 在 addon.cc 文件中，使用 node-addon-api 的 API 来编写你的 C++ 代码  
...
6. 创建binding.gyp， 在 binding.gyp 文件中定义你的 API：  
...
会下载一堆东西,比如node.lib，放在`C:\\Users\\Administrator\\AppData\\Local\\node-gyp\\Cache\\16.17.0`里。  
...
8. 测试和调试: 使用 Node.js 运行你的插件。例如，创建一个名为 test.js 的文件，并编写以下代码：  
...
9. 在项目根目录中运行以下命令：
 ...
 windows下npm全局安裝的包，可能不能被node正确找到，报错Cannot find module node-addon-api。
