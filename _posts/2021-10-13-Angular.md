...
## 基本概念  
### 组件  
组件是 Angular 应用的主要构建块。每个组件都代表一个大型网页的一部分。组件包括三个部分：  
...
从Angular 17(2023年11月)开始  
+ 默认使用 Standalone Components, 使用以下命令来生成一个旧模式的project

        ng new my-angular-app --standalone=false
        
+ 新项目默认不再生成 app.module.ts
+ 使用 app.config.ts 替代模块配置
+ 使用 bootstrapApplication() 替代 platformBrowserDynamic().bootstrapModule()

[独立组件入门](https://v17.angular.cn/guide/standalone-components)  

### Signal
[Angular Signals 完全指南](https://juejin.cn/post/7404780384539983924)  