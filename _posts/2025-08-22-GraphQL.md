...
但是<strong>使用type定义出来的数据类型只能作为查询类型，却不能作为请求的输入类型</strong>。所以我们还需要使用`input`定义输入类型AccountInput作为请求的输入参数：  
    ...
    input AccountInput {
        name: String   # 只传真正需要的字段
        age: Int
        city: String
    }

这是 GraphQL 类型系统的刻意设计， 这种分离实际上强制你思考哪些字段是客户端应该提供的，哪些是服务端负责的。 
...
# Apollo
GraphQL 和 Apollo 是规范与实现的关系。GraphQL 是一种查询语言规范， 它只定义了：

+ 查询语法（query、mutation、subscription）
+ Schema 定义语言（type、input、interface 等）
+ 执行规则  

规范本身不是代码，需要具体的库来实现它。  
Apollo 是目前最流行的 GraphQL 生态工具集，提供了规范的完整实现。
...
2. 安装依赖
    ...
    注意：`apollo-server` 包已被废弃，官方推荐迁移至 `@apollo/server`（Apollo Server v4）。此处示例仍使用 `apollo-server` v3，如需使用最新版本请参考 [Apollo Server 官方文档](https://www.apollographql.com/docs/apollo-server/getting-started)。
...
TypeGraphQL在Github里提供了很多实例。[Github](https://github.com/MichalLytek/type-graphql/tree/v2.0.0-rc.4/examples)
