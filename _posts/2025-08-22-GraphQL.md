...
TypeGraphQL是一个库，它仅使用类和一些装饰器decorator来定义schema，使定义graphQL这个过程变得轻松愉快。
...
在 Node.js 中开发 GraphQL API 遇到的最大问题是代码冗余。要向entity添加新field，我们必须遍历所有文件：修改entity类，然后修改schema，最后更新接口。很容易忘记更新其中一个，或者在类型上犯错。  

TypeGraphQL正是基于多年来使用 TypeScript 开发 GraphQL API 的经验，旨在解决这些问题。其核心思想是通过使用类和一些装饰器来定义schema，从而实现单一真实来源（one source of truth）。  


https://github.com/MichalLytek/type-graphql/tree/v2.0.0-rc.2/examples
